...
public sealed class LoginQueueTracker : IDisposable
{
    ...
    public static TimeSpan? NextIdentifyTime =>
        LoginQueue.AgentLobbyGetTimeSinceLastIdentify() is { } lastTime ?
            TimeSpan.FromSeconds(30) - TimeSpan.FromMilliseconds(lastTime) :
            null;

    private DateTime? LastIdentifyTime { get; set; }

    public LoginQueueTracker()
    {
        Service.Hooks.Login.OnEnterQueue += OnEnterQueue;
        Service.Hooks.Login.OnCancelQueue += OnCancelQueue;
        Service.Hooks.Login.OnFailedQueue += OnFailedQueue;
        Service.Hooks.Login.OnExitQueue += OnExitQueue;
        Service.Hooks.Login.OnSendIdentify += OnSendIdentify;
        Service.Hooks.Login.OnNewQueuePosition += OnNewQueuePosition;
